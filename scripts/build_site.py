...
REPOSITORY_URL: Final[str] = "https://github.com/kogunlowo123/prompt-injection-firewall"
...
PAGES: Final[tuple[Page, ...]] = (
    ...
    Page(Path("docs/detection.md"), "detection", "The detection stack", "Reference"),
    Page(Path("docs/attacks.md"), "attacks", "The attack corpus", "Reference"),
    Page(Path("docs/false-positives.md"), "false-positives", "False positives", "Reference"),
    Page(Path("docs/policy.md"), "policy", "Policy and the audit record", "Reference"),
    Page(Path("docs/reproducibility.md"), "reproducibility", "Reproducibility", "Reference"),
    Page(Path("docs/ci.md"), "ci", "Continuous integration", "Reference"),
...
def build(output: Path) -> int:
    ...
    for page in PAGES:
        ...
        (output / page.output_name).write_text(
            TEMPLATE.format(
                title=html.escape("pifw" if page.slug == "index" else f"{page.title} — pifw"),
